...
- Write a real-account listing verification ladder. Step 0 asks Hostaway
  support for authoritative `PUT` semantics and remains incomplete until an
  authoritative response is recorded. Step 1 captures a complete target
  ...
  disposable task canary, snapshots it, sends partial `PUT /v1/tasks/{id}`,
  verifies unrelated task fields survive, applies the allowlisted restore
  payload built by the same production restore-path code used by Steps 4 and
  5, verifies the task matches its pre-mutation snapshot, and deletes the
  task; this proves server-accepted restore for the disposable task path but is
  still indicative, not conclusive, for listing semantics because Hostaway may
