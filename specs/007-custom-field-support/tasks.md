...
- [ ] T007 Add `CustomFieldWriteSafetyGates` defaults with `listing_partial_put_verified = False` and `reservation_no_clobber_verified = False` in `custom_components/hostaway/api/custom_fields.py`; do not add any code path that can override them yet
...
- [ ] T009 [P] [API] Write failing tests in `tests/api/test_custom_fields.py` for `HostawayCustomFieldValue` and `HostawayCustomFieldCollection`, covering present values, explicit `value: None`, present empty `[]`, missing `customFieldValues`, `null`, non-list invalid state, malformed entries skipped for presentation, and raw malformed entry preservation
- [ ] T010 [P] [API] Write failing tests in `tests/api/test_custom_fields.py` for duplicate raw entries and addressed malformed-entry detection, including malformed raw entries with the addressed `customFieldId`
- [ ] T011 [P] [API] Write failing tests in `tests/api/test_custom_fields.py` for paginated `GET /v1/customFields?limit=500&offset=...`, stopping by pagination metadata or short page, and filtering out `task` definitions
- [ ] T012 [P] [API] Write failing tests in `tests/api/test_custom_fields.py` proving listing page reads, reservation page reads, direct listing reads, and direct reservation reads all include `includeResources=1`
- [ ] T013 [P] [API] Write failing tests in `tests/api/test_custom_fields.py` proving reservation pagination stays keyed off raw reservation ids when `includeResources=1` is added

### Implementation for Foundational API

- [ ] T014 [API] Implement bool-safe integer validation helpers in `custom_components/hostaway/api/custom_fields.py` for definition ids, value-entry `customFieldId`, service `customFieldId`, and `target_id`; reject `True` and `False` everywhere identifiers are expected
- [ ] T015 [API] Implement `HostawayCustomFieldDefinition` parsing and response-shaping helpers in `custom_components/hostaway/api/custom_fields.py`
- [ ] T016 [API] Implement `HostawayCustomFieldValue` and `HostawayCustomFieldCollection` in `custom_components/hostaway/api/custom_fields.py`, preserving raw entries and distinguishing `present`, `missing`, `null`, and `invalid`
...
- [ ] T020 [API] Update listing pagination in `custom_components/hostaway/api/client.py` with the smallest possible change to request `includeResources=1` while keeping the file under 400 lines
...
- [ ] T024 [P] [Coordinator] Write failing tests in `tests/test_init.py` or the existing setup tests proving `hass.data[DOMAIN][entry.entry_id]` remains the existing dict and receives new dedicated keys for `custom_fields_coordinator`, `custom_field_write_safety`, write locks, and write generations
...
- [ ] T032 [Coordinator] Wire `HostawayCustomFieldsCoordinator` into `custom_components/hostaway/__init__.py` under a new key inside `hass.data[DOMAIN][entry.entry_id]` without replacing that mapping, and rely on the existing unload path to remove it with the per-entry data
...
- [ ] T053 [P] [US2] Write failing tests in `tests/services/test_custom_fields.py` proving missing `config_entry_id` fails closed with `config_entry_id required when multiple entries exist` before any read when multiple entries are loaded
