...
- [ ] T096 [API] Write failing tests for the full-object payload builder, per-target writable-field allowlists and normalization rules, rejection of unsafe GET-response deep copies, per-target strategy selection, explicit payload-strategy gate state, `--snapshot` mode, allowlisted restore-payload reconstruction, canonicalized complete-snapshot comparison, account-bound reservation evidence, reservation fail-closed behavior when existing reservation `customFieldValues` are present without round-trip evidence, and task-canary mode in `tests/api/test_custom_fields.py` and `tests/scripts/test_verify_custom_field_writes.py` before implementing T093 through T095
